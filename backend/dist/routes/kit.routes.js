...
function stringArray(value, max) {
    return Array.isArray(value)
        ? value.filter((item) => typeof item === 'string' && item.trim().length > 0).slice(0, max)
        : [];
}
async function validateOwnedReferences(userId, assetIds, projectIds) {
...
router.put('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (!current)
            return void res.status(404).json({ error: 'Kit not found' });
        const name = typeof req.body.name === 'string' ? req.body.name.trim() : '';
        const kind = req.body.kind === 'character' ? 'character' : req.body.kind === 'brand' ? 'brand' : '';
        const assetIds = [...new Set(stringArray(req.body.asset_ids, 12))];
        const projectIds = [...new Set(stringArray(req.body.project_ids, 30))];
        if (!name || name.length > 120 || !kind)
            return void res.status(400).json({ error: 'A valid name and kit type are required' });
        if (!(await validateOwnedReferences(req.user.id, assetIds, projectIds)))
            return void res.status(404).json({ error: 'One or more assets or projects were not found' });
