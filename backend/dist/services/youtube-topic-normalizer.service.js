...
exports.normalizeYoutubeIdea = normalizeYoutubeIdea;
exports.isMalformedGeneratedText = isMalformedGeneratedText;
exports.cleanGeneratedTitle = cleanGeneratedTitle;
...
function labeledValue(raw, labels) {
...
function unique(values, limit = 12) {
    ...
    return values.filter((value) => {
        ...
        if (!value || STOP_WORDS.has(key) || seen.has(key))
            return false;
        seen.add(key);
        return true;
    }).slice(0, limit);
}
function extractCapitalizedPhrases(raw) {
...
function conceptPhrases(raw) {
    ...
    if (/chemical weapon|weapon so toxic|poison gas|wartime chem/.test(lower))
        matches.push('chemical warfare');
    if (/antibiotic|life-saving medicine|saved millions of lives/.test(lower))
        matches.push('antibiotics');
    if (/pharmaceutical|medicinal chem/.test(lower))
        matches.push('pharmaceutical chemistry');
    if (/\b1914\b|first world war|world war i/.test(lower))
        matches.push('World War I');
    const quoted = Array.from(raw.matchAll(/["“]([^"”]{3,70})["”]/g)).map((match) => tidy(match[1], 70));
    return unique([...matches, ...quoted], 10);
}
function deriveTopic(title, hook, concepts) {
    ...
    if (concepts.length >= 2)
        return `The relationship between ${concepts[0]} and ${concepts[1]}`;
    if (concepts.length === 1)
        return `The hidden consequences and contested history of ${concepts[0]}`;
    return title.replace(/^(The|A|An)\s+/i, 'The history and consequences of ');
}
function deriveTension(topic, title) {
...
function normalizeYoutubeIdea(rawValue, statedTakeaway) {
    ...
    const viewerTakeaway = sentence(labeledValue(rawIdea, ['viewer takeaway', 'takeaway']) || statedTakeaway || '', takeawayFromTension.startsWith('Viewers should')
        ? takeawayFromTension
...
function isMalformedGeneratedText(value, rawIdea) {
    const text = tidy(String(value || ''), 2_000);
    if (!text || text.split(/\s+/).length < 4)
        return true;
    if ((text.match(/"/g) || []).length % 2 !== 0)
        return true;
    if (/(?:\b(?:and|but|because|while|that|which|who|to|of|a|an|the))$/i.test(text))
        return true;
    if (/^(?:\d+\s*[.)-]|(?:the\s+)?hook\s*:)/i.test(text))
        return true;
    if (rawIdea && rawIdea.length > 80 && text.includes(rawIdea))
        return true;
    return false;
}
function cleanGeneratedTitle(value, fallback) {
    ...
    if (title.length < 4 || words.length < 2 || repeated || /(?:the\s+)?hook\s*:/i.test(String(value || '')))
        return fallback;
    return title;
}
