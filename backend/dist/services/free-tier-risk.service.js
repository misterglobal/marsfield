...
exports.GenerationGateError = void 0;
exports.hashRiskSignal = hashRiskSignal;
exports.normalizePromptForRisk = normalizePromptForRisk;
exports.companyDomain = companyDomain;
exports.clientIp = clientIp;
exports.recordRiskSignal = recordRiskSignal;
exports.recordRegistrationSignals = recordRegistrationSignals;
...
exports.refundUnacceptedCredits = refundUnacceptedCredits;
exports.initializeFreeTierRiskControls = initializeFreeTierRiskControls;
...
function hashRiskSignal(type, value) {
    return (0, crypto_1.createHash)('sha256').update(`${signalPepper()}:${type}:${value.trim().toLowerCase()}`).digest('hex');
}
function normalizePromptForRisk(prompt) {
...
async function recordRiskSignal(userId, type, rawValue, client = prisma) {
    if (!rawValue)
        return;
...
async function recordRegistrationSignals(userId, email, req, client = prisma) {
...
async function recordGenerationSignals(userId, prompt, req) {
    await Promise.all([
...
async function getRiskContext(userId, client = prisma) {
    const shared = await client.$queryRaw `
    SELECT rs.type,
           rs.id AS "signalId",
...
class GenerationGateError extends Error {
    status;
    code;
    details;
...
exports.GenerationGateError = GenerationGateError;
function effectiveDuration(params) {
...
function assertGenerationPolicy(input) {
...
function estimateCostMicros(credits) {
    return BigInt(Math.ceil(credits * numberEnv('GENERATION_COST_PER_CREDIT_USD', 0.032) * 1_000_000));
}
function maxConcurrentJobs(plan) {
    const defaults = { free: 1, starter: 3, creator: 5, pro: 10, studio: 20 };
    return Math.max(1, Math.floor(numberEnv(`MAX_CONCURRENT_JOBS_${plan.toUpperCase()}`, defaults[plan] || 2)));
}
function utcBudgetDate() {
    return new Date(`${new Date().toISOString().slice(0, 10)}T00:00:00.000Z`);
}
async function reserveInTransaction(input, risk) {
    const isFree = input.user.plan.toLowerCase() === 'free';
    ...
    const requestKey = (0, crypto_1.randomUUID)();
    ...
    return prisma.$transaction(async (tx) => {
        ...
        const concurrencyRows = await tx.$queryRaw `
      SELECT (
        (SELECT COUNT(*) FROM predictions
          WHERE user_id IN (${client_1.Prisma.join(subjectIds)})
    ...
    `;
        ...
        if (isFree) {
            const lifetimeRows = await tx.$queryRaw `
    ...
    `;
        if (budgetRows.length !== 1) {
            throw new GenerationGateError(isFree ? 'Today\'s free generation budget has been reached. Free usage resumes at 00:00 UTC.' : 'The platform generation safety limit has been reached.', 503, isFree ? 'free_daily_budget_exhausted' : 'platform_safety_budget_exhausted');
        }
...
async function finalizeGenerationReservation(reservation, acceptedCredits) {
...
async function refundUnacceptedCredits(userId, isFree, credits) {
    if (credits <= 0)
        return;
...
async function initializeFreeTierRiskControls() {
