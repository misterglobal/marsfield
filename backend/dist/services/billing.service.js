...
function getBaseCredits(input) {
    ...
    if (input.workflow === 'image-upscale') {
        ...
        if (!Number.isFinite(outputMegapixels) || outputMegapixels <= 0)
            throw new Error('Could not calculate upscale output size');
    ...
    if (input.workflow === 'video-enhance') {
        ...
        if (!Number.isFinite(duration) || duration <= 0)
            throw new Error('Could not calculate enhancement duration');
