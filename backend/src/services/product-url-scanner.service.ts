...
async function safeUrl(value: string) {
  let url: URL;
  try { url = new URL(value); } catch { throw new Error('Enter a valid HTTPS product URL'); }
  if (url.protocol !== 'https:' || url.username || url.password || (url.port && url.port !== '443')) throw new Error('Product URLs must use public HTTPS without embedded credentials or custom ports');
  if (url.hostname === 'localhost' || url.hostname.endsWith('.local')) throw new Error('Local and private-network URLs are not allowed');
  const addresses = await lookup(url.hostname, { all: true, verbatim: true });
  if (!addresses.length || addresses.some(({ address }) => privateAddress(address))) throw new Error('Product URL resolves to a private or unsafe network address');
  return { url, address: addresses[0] };
}

async function fetchHtml(value: string, redirects = 0): Promise<{ html: string; finalUrl: string }> {
  if (redirects > 3) throw new Error('Product URL redirected too many times');
  ...
  return new Promise((resolve, reject) => {
    const request = httpsRequest(url, {
      ...
      if (response.statusCode && response.statusCode >= 300 && response.statusCode < 400 && location) {
        response.resume(); void fetchHtml(new URL(location, url).toString(), redirects + 1).then(resolve, reject); return;
      }
      if (!response.statusCode || response.statusCode < 200 || response.statusCode >= 300) { response.resume(); reject(new Error(`Product page returned HTTP ${response.statusCode || 'error'}`)); return; }
      const contentType = String(response.headers['content-type'] || '').toLowerCase();
      if (!contentType.includes('text/html') && !contentType.includes('application/xhtml+xml')) { response.resume(); reject(new Error('URL does not point to an HTML product page')); return; }
      const declared = Number(response.headers['content-length'] || 0);
      if (declared > MAX_HTML_BYTES) { response.resume(); reject(new Error('Product page is too large to scan')); return; }
      const chunks: Buffer[] = []; let size = 0;
      response.on('data', (chunk: Buffer) => { size += chunk.length; if (size > MAX_HTML_BYTES) request.destroy(new Error('Product page is too large to scan')); else chunks.push(chunk); });
...
function metadata(html: string, baseUrl: string) {
  ...
  for (const match of html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
    try { jsonLd.push(JSON.parse(match[1].trim())); } catch { /* malformed merchant markup */ }
  }
  const title = decode(html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1] || '');
  const visibleText = decode(html.replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' ')).slice(0, 12_000);
  const imageCandidates = [metas['og:image'], metas['twitter:image']].filter(Boolean).map((item) => { try { return new URL(item, baseUrl).toString(); } catch { return ''; } }).filter((item) => item.startsWith('https://'));
  return { title, metas, json_ld: jsonLd.slice(0, 12), visible_text: visibleText, image_candidates: imageCandidates };
}

export async function scanProductUrl(sourceUrl: string) {
  const fetched = await fetchHtml(sourceUrl);
  const extracted = metadata(fetched.html, fetched.finalUrl);
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error('Product scanning is not configured');
  ...
  if (!response.ok) throw new Error(payload?.error?.message || 'OpenAI product extraction failed');
  ...
  if (!outputText) throw new Error('Product scanner returned no data');
  const result = JSON.parse(outputText);
  const safeImages: string[] = [];
  for (const item of (result.image_urls || []).slice(0, 8)) {
    try { const candidate = new URL(item, fetched.finalUrl).toString(); await safeUrl(candidate); safeImages.push(candidate); } catch { /* discard unsafe image URL */ }
  }
