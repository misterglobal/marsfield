...
async function structuredResponse(instructions: string, input: string, schema: Record<string, unknown>) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error('Scene planning is not configured. Set OPENAI_API_KEY.');
  ...
  if (!response.ok) throw new Error(payload?.error?.message || 'OpenAI scene planning failed');
  ...
  if (!outputText) throw new Error('OpenAI returned no scene plan');
  return JSON.parse(outputText);
}

export async function planInfluencerScenes(input: {
  idea: string; title: string; durationSeconds: number; aspectRatio: string; brief: unknown;
  influencer: unknown; product?: unknown; sceneCount: number;
}) {
  const result = await structuredResponse(
    'You are a social video director. Return a practical, chronological scene plan. Keep one adult fictional influencer visually consistent. Scene durations must be positive integers and their sum must exactly equal the requested duration. Use at most one product. Write dialogue or voiceover that can be naturally spoken in the allotted time. Do not include markdown.',
  ...
  const scenes = result.scenes as InfluencerScene[];
  ...
  if (total !== input.durationSeconds) throw new Error('Planner scene durations do not match the project duration');
  return result;
}

export async function rewriteInfluencerScene(scene: InfluencerScene, instruction: string, context: unknown) {
  return structuredResponse(
    'Rewrite one social-video scene according to the instruction. Preserve its duration_seconds exactly. Return only the complete scene object.',
    JSON.stringify({ scene, instruction, context }),
    sceneSchema,
  ) as Promise<InfluencerScene>;
}
