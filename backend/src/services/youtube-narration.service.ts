...
export interface YoutubeNarrationPackageInput {
  script: unknown;
  storyboard: unknown;
...
function cleanNarration(value: unknown): string {
  return String(value || '')
    .replace(/\[[^\]]+\]/g, '')
...
export function createYoutubeNarrationPackage(input: YoutubeNarrationPackageInput) {
  const script = (input.script || {}) as { sections?: ScriptSection[]; ttsText?: string };
  ...
  if (!narrationText) throw new Error('Production script has no narration text');
  ...
  const storyboard = Array.isArray(input.storyboard) ? input.storyboard as StoryboardScene[] : [];
