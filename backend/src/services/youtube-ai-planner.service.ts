...
import { cleanGeneratedTitle, isMalformedGeneratedText } from './youtube-topic-normalizer.service';
...
const YOUTUBE_AI_TIMEOUT_MS = Math.max(
  1_000,
  Math.min(25_000, Number(process.env.YOUTUBE_AI_TIMEOUT_MS) || 15_000),
);
  ...
  try {
    ...
    return {
      ...
      storyboard: baseline.storyboard.map((scene: any, index: number) => {
        ...
        const safe = (value: unknown, fallback: string) => isMalformedGeneratedText(value, String(input.topic || '')) ? fallback : String(value).trim();
        return {
          ...scene,
          title: cleanGeneratedTitle(ai.title, scene.title),
