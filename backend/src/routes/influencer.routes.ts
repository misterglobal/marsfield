...
import { planInfluencerScenes, rewriteInfluencerScene, InfluencerScene } from '../services/influencer-planner.service';
import { scanProductUrl } from '../services/product-url-scanner.service';
...
const list = (value: unknown) => Array.isArray(value) ? value.map((item) => text(item, 200)).filter(Boolean).slice(0, 20) : [];
const json = (value: unknown) => value as Prisma.InputJsonValue;

function influencerPrompt(name: string, profile: any, hasReferences = false) {
...
function sceneData(body: any, index: number) {
  const durationSeconds = Number(body.duration_seconds);
  if (!Number.isInteger(durationSeconds) || durationSeconds < 1 || durationSeconds > 15) throw new Error('Scene duration must be 1–15 seconds');
  return {
...
function storyboardLayout(sceneCount: number) {
  if (sceneCount === 3) return '3x1';
  if (sceneCount === 4) return '2x2';
  return '3x2';
}

function storyboardPrompt(project: Awaited<ReturnType<typeof ownedProject>>, visualStyle: string, consistency: string) {
  if (!project) throw new Error('Influencer project not found');
...
function klingPrompt(project: NonNullable<Awaited<ReturnType<typeof ownedProject>>>, motionInstructions: string) {
  const storyboard = project.storyboards.find((item) => item.approved);
  if (!storyboard) throw new Error('Approve a storyboard before video generation');
...
router.get('/influencers', authMiddleware, requireScope('projects:read'), async (req: AuthenticatedRequest, res: Response) => {
...
router.post('/influencers', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const name = text(req.body.name, 80); const profile = req.body.profile;
    if (!name || !profile || typeof profile !== 'object' || Array.isArray(profile)) return void res.status(400).json({ error: 'Name and influencer profile are required' });
    const age = Number((profile as any).age);
    if (!Number.isInteger(age) || age < 18 || age > 80) return void res.status(400).json({ error: 'Influencer age must be between 18 and 80' });
    const referenceIds = list(req.body.reference_storage_ids).slice(0, 4);
    if (referenceIds.length && req.body.reference_consent !== true) return void res.status(400).json({ error: 'Consent confirmation is required for uploaded people' });
    if (referenceIds.length) {
      const owned = await prisma.storageObject.count({ where: { id: { in: referenceIds }, userId: req.user!.id, mimeType: { startsWith: 'image/' } } });
      if (owned !== referenceIds.length) return void res.status(400).json({ error: 'Influencer references must be owned image uploads' });
    }
  ...
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed creating influencer' }); }
});

router.get('/influencers/:id/generation-prompt', authMiddleware, requireScope('projects:read'), async (req: AuthenticatedRequest, res: Response) => {
  const row = await prisma.influencer.findFirst({ where: { id: req.params.id, userId: req.user!.id } });
  if (!row) return void res.status(404).json({ error: 'Influencer not found' });
  const referenceIds = Array.isArray(row.referenceStorageIds) ? row.referenceStorageIds.filter((id): id is string => typeof id === 'string').slice(0, 4) : [];
  res.json({ prompt: influencerPrompt(row.name, row.profile, referenceIds.length > 0), reference_storage_ids: referenceIds });
});

router.post('/influencers/:id/approve', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const influencer = await prisma.influencer.findFirst({ where: { id: req.params.id, userId: req.user!.id } });
    if (!influencer) return void res.status(404).json({ error: 'Influencer not found' });
    ...
    if (!prediction?.assets[0]) return void res.status(400).json({ error: 'A completed Nano Banana 2 image is required' });
    ...
    const result = await prisma.$transaction(async (tx) => {
      const created = await tx.influencerVersion.create({ data: { influencerId: influencer.id, version, profile: influencer.profile as Prisma.InputJsonValue, canonicalDescription: description, voiceProfile: influencer.voiceProfile, generationPrompt: prediction.prompt || '', predictionId: prediction.id, assetId: prediction.assets[0].id, imageUrl: prediction.outputUrl } });
  ...
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed approving influencer' }); }
});

router.get('/products', authMiddleware, requireScope('projects:read'), async (req: AuthenticatedRequest, res: Response) => {
...
router.post('/products/scan-url', authMiddleware, requireScope('projects:write'), rateLimit('quote'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    res.json(await scanProductUrl(url));
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed scanning product URL' }); }
});

router.post('/products', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const name = text(req.body.name, 120); const ids = list(req.body.reference_storage_ids).slice(0, 8);
    if (!name) return void res.status(400).json({ error: 'Product name is required' });
    if (ids.length) {
      const owned = await prisma.storageObject.count({ where: { id: { in: ids }, userId: req.user!.id, mimeType: { startsWith: 'image/' } } });
      if (owned !== ids.length) return void res.status(400).json({ error: 'Product references must be owned image uploads' });
    }
    const product = await prisma.influencerProduct.create({ data: { userId: req.user!.id, name, description: text(req.body.description, 2000) || null, brandName: text(req.body.brand_name, 120) || null, approvedClaims: json(list(req.body.approved_claims)), restrictedClaims: json(list(req.body.restricted_claims)), brandColours: json(list(req.body.brand_colours)), referenceStorageIds: json(ids), primaryStorageObjectId: ids[0] || null, sourceUrl: text(req.body.source_url, 2000) || null, sourceData: req.body.source_data ? json(req.body.source_data) : Prisma.JsonNull, externalImageUrls: json(list(req.body.external_image_urls)) } });
    res.status(201).json(product);
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed creating product' }); }
});

router.get('/projects', authMiddleware, requireScope('projects:read'), async (req: AuthenticatedRequest, res: Response) => {
...
router.get('/projects/:id', authMiddleware, requireScope('projects:read'), async (req: AuthenticatedRequest, res: Response) => {
...
router.post('/projects', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!influencer?.versions[0]) return void res.status(400).json({ error: 'Select an approved influencer' });
    const productId = text(req.body.product_id, 100) || null;
    if (productId && !await prisma.influencerProduct.findFirst({ where: { id: productId, userId: req.user!.id } })) return void res.status(400).json({ error: 'Product not found' });
    const durationSeconds = Number(req.body.duration_seconds); const aspectRatio = text(req.body.aspect_ratio, 10);
    if (!durations.has(durationSeconds) || !ratios.has(aspectRatio)) return void res.status(400).json({ error: 'Choose a supported duration and aspect ratio' });
    const title = text(req.body.title, 120); const idea = text(req.body.idea, 4000);
    if (!title || !idea) return void res.status(400).json({ error: 'Project title and idea are required' });
...
router.post('/projects/:id/plan', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.influencerVideoProject.update({ where: { id: project.id }, data: { status: 'scenes_draft', scenesRevision: { increment: 1 }, scenesApprovedAt: null, brief: json({ ...(project.brief as object), plan_summary: { creative_angle: plan.creative_angle, hook: plan.hook, narrative_structure: plan.narrative_structure, script: plan.script, product_integration: plan.product_integration, call_to_action: plan.call_to_action } }) } });
    });
    res.status(201).json(await ownedProject(req.user!.id, project.id));
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed planning scenes' }); }
});

router.patch('/projects/:id/scenes/:sceneId', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const project = await ownedProject(req.user!.id, req.params.id);
    const existing = project?.scenes.find((scene) => scene.id === req.params.sceneId);
    if (!project || !existing) return void res.status(404).json({ error: 'Scene not found' });
  ...
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed updating scene' }); }
});

router.post('/projects/:id/scenes', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const project = await ownedProject(req.user!.id, req.params.id);
    if (!project) return void res.status(404).json({ error: 'Influencer project not found' });
    if (project.scenes.length >= 6) return void res.status(400).json({ error: 'Projects are limited to six scenes' });
    ...
    res.status(201).json(await ownedProject(req.user!.id, project.id));
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed adding scene' }); }
});

router.delete('/projects/:id/scenes/:sceneId', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  const project = await ownedProject(req.user!.id, req.params.id);
  const existing = project?.scenes.find((scene) => scene.id === req.params.sceneId);
  if (!project || !existing) return void res.status(404).json({ error: 'Scene not found' });
  if (project.scenes.length <= 2) return void res.status(400).json({ error: 'Projects require at least two scenes' });
  await prisma.$transaction(async (tx) => {
...
router.post('/projects/:id/scenes/:sceneId/duplicate', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const project = await ownedProject(req.user!.id, req.params.id); const source = project?.scenes.find((scene) => scene.id === req.params.sceneId);
    if (!project || !source) return void res.status(404).json({ error: 'Scene not found' });
    if (project.scenes.length >= 6) return void res.status(400).json({ error: 'Projects are limited to six scenes' });
    await prisma.$transaction(async (tx) => {
      ...
      for (const scene of shifted) await tx.influencerProjectScene.update({ where: { id: scene.id }, data: { index: scene.index + 1 } });
    ...
    res.status(201).json(await ownedProject(req.user!.id, project.id));
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed duplicating scene' }); }
});

router.post('/projects/:id/scenes/reorder', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const project = await ownedProject(req.user!.id, req.params.id); const ids = list(req.body.scene_ids);
    if (!project || ids.length !== project.scenes.length || new Set(ids).size !== ids.length || ids.some((id) => !project.scenes.some((scene) => scene.id === id))) return void res.status(400).json({ error: 'Provide every project scene exactly once' });
    await prisma.$transaction(async (tx) => {
      await tx.influencerProjectScene.updateMany({ where: { projectId: project.id }, data: { index: { increment: 100 } } });
      for (let index = 0; index < ids.length; index += 1) await tx.influencerProjectScene.update({ where: { id: ids[index] }, data: { index } });
      await tx.influencerVideoProject.update({ where: { id: project.id }, data: { status: 'scenes_draft', scenesApprovedAt: null, scenesRevision: { increment: 1 } } });
    });
    res.json(await ownedProject(req.user!.id, project.id));
...
router.post('/projects/:id/scenes/:sceneId/rewrite', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!project || !existing) return void res.status(404).json({ error: 'Scene not found' });
    if (!instruction) return void res.status(400).json({ error: 'Rewrite instruction is required' });
    const source = { ...existing, duration_seconds: existing.durationSeconds, character_action: existing.characterAction, camera_framing: existing.cameraFraming, camera_movement: existing.cameraMovement, product_visible: existing.productVisible, on_screen_text: existing.onScreenText } as unknown as InfluencerScene;
    const rewritten = await rewriteInfluencerScene(source, instruction, { project: project.brief, product: project.product });
    const updated = sceneData({ ...rewritten, duration_seconds: existing.durationSeconds }, existing.index);
    await prisma.$transaction([prisma.influencerProjectScene.update({ where: { id: existing.id }, data: updated }), prisma.influencerVideoProject.update({ where: { id: project.id }, data: { status: 'scenes_draft', scenesApprovedAt: null, scenesRevision: { increment: 1 } } })]);
    res.json(await ownedProject(req.user!.id, project.id));
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed rewriting scene' }); }
});

router.post('/projects/:id/scenes/approve', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  const project = await ownedProject(req.user!.id, req.params.id);
  if (!project) return void res.status(404).json({ error: 'Influencer project not found' });
  if (project.scenes.length < 2 || project.scenes.length > 6) return void res.status(400).json({ error: 'Projects require 2–6 scenes' });
  if (project.scenes.reduce((sum, scene) => sum + scene.durationSeconds, 0) !== project.durationSeconds) return void res.status(400).json({ error: 'Scene durations must equal the project duration' });
  await prisma.influencerVideoProject.update({ where: { id: project.id }, data: { status: 'scenes_approved', scenesApprovedAt: new Date() } });
  res.json(await ownedProject(req.user!.id, project.id));
});

router.post('/projects/:id/storyboards/prepare', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const project = await ownedProject(req.user!.id, req.params.id);
    if (!project) return void res.status(404).json({ error: 'Influencer project not found' });
    if (!['scenes_approved', 'storyboard_ready', 'storyboard_approved', 'video_ready'].includes(project.status)) return void res.status(409).json({ error: 'Approve the current scene plan before generating a storyboard' });
    ...
    if (!influencerAsset?.storageObjectId) return void res.status(409).json({ error: 'The approved influencer portrait must be stored in durable storage before storyboard generation' });
    const productIds = Array.isArray(project.product?.referenceStorageIds) ? project.product.referenceStorageIds.filter((id): id is string => typeof id === 'string').slice(0, 3) : [];
  ...
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed preparing storyboard' }); }
});

router.post('/projects/:id/storyboards', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const project = await ownedProject(req.user!.id, req.params.id);
    if (!project) return void res.status(404).json({ error: 'Influencer project not found' });
    if (project.status !== 'scenes_approved' && project.status !== 'storyboard_ready' && project.status !== 'storyboard_approved') return void res.status(409).json({ error: 'The current scene plan is not approved' });
    if (Number(req.body.scenes_revision) !== project.scenesRevision) return void res.status(409).json({ error: 'Scenes changed after this storyboard generation started' });
    const prediction = await prisma.prediction.findFirst({ where: { id: text(req.body.prediction_id, 100), userId: req.user!.id, workflow: 'text-to-image', model: 'google/nano-banana-2', status: 'succeeded' }, include: { assets: { where: { type: 'image' }, take: 1 } } });
    if (!prediction?.assets[0] || !prediction.outputUrl) return void res.status(400).json({ error: 'A completed Nano Banana 2 storyboard image is required' });
    ...
    const created = await prisma.$transaction(async (tx) => {
      const storyboard = await tx.influencerStoryboardVersion.create({ data: { projectId: project.id, version, scenesRevision: project.scenesRevision, sceneSnapshot: json(project.scenes), influencerSnapshot: json(project.influencerVersion), productSnapshot: project.product ? json(project.product) : Prisma.JsonNull, prompt: text(req.body.prompt, 4000) || prediction.prompt || '', layout: text(req.body.layout, 20) || storyboardLayout(project.scenes.length), visualStyle: text(req.body.visual_style, 300) || 'premium UGC', referenceStorageIds: json(list(req.body.reference_storage_ids)), predictionId: prediction.id, assetId: prediction.assets[0].id, imageUrl: storyboardImageUrl } });
      await tx.influencerVideoProject.update({ where: { id: project.id }, data: { status: 'storyboard_ready' } });
      return storyboard;
    });
    res.status(201).json(created);
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed saving storyboard version' }); }
});

router.post('/projects/:id/storyboards/:storyboardId/approve', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  const project = await ownedProject(req.user!.id, req.params.id);
  const storyboard = project?.storyboards.find((item) => item.id === req.params.storyboardId);
  if (!project || !storyboard) return void res.status(404).json({ error: 'Storyboard version not found' });
  if (storyboard.scenesRevision !== project.scenesRevision || !project.scenesApprovedAt) return void res.status(409).json({ error: 'This storyboard no longer matches the approved scene plan' });
...
router.post('/projects/:id/videos/prepare', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const project = await ownedProject(req.user!.id, req.params.id);
    if (!project) return void res.status(404).json({ error: 'Influencer project not found' });
    if (project.status !== 'storyboard_approved' && project.status !== 'video_ready') return void res.status(409).json({ error: 'Approve a storyboard before generating video' });
    const storyboard = project.storyboards.find((item) => item.approved);
    if (!storyboard || storyboard.scenesRevision !== project.scenesRevision) return void res.status(409).json({ error: 'The approved storyboard no longer matches the scenes' });
    ...
    if (!storyboardAsset?.storageObjectId) return void res.status(409).json({ error: 'The approved storyboard must be in durable storage' });
    const influencerAsset = await prisma.asset.findFirst({ where: { id: project.influencerVersion.assetId || '', userId: req.user!.id }, select: { storageObjectId: true } });
    if (!influencerAsset?.storageObjectId) return void res.status(409).json({ error: 'The approved influencer portrait must be in durable storage' });
    const productIds = Array.isArray(project.product?.referenceStorageIds) ? project.product.referenceStorageIds.filter((id): id is string => typeof id === 'string').slice(0, 5) : [];
  ...
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed preparing Kling V3 video' }); }
});

router.post('/projects/:id/videos', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const project = await ownedProject(req.user!.id, req.params.id);
    if (!project) return void res.status(404).json({ error: 'Influencer project not found' });
    const storyboard = project.storyboards.find((item) => item.id === text(req.body.storyboard_id, 100) && item.approved);
    if (!storyboard || storyboard.scenesRevision !== project.scenesRevision) return void res.status(409).json({ error: 'Approved storyboard not found or stale' });
    ...
    if (!prediction?.assets[0] || !prediction.outputUrl) return void res.status(400).json({ error: 'A completed Kling V3 video is required' });
    const existing = await prisma.influencerVideoGeneration.findUnique({ where: { predictionId: prediction.id } });
    if (existing) return void res.json(existing);
    const videoUrl = prediction.outputUrl; const attempt = (project.videoGenerations[0]?.attempt || 0) + 1;
    const created = await prisma.$transaction(async (tx) => {
    ...
    res.status(201).json(created);
  } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Failed saving video generation' }); }
});

router.post('/projects/:id/videos/:videoId/approve', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  const project = await ownedProject(req.user!.id, req.params.id); const video = project?.videoGenerations.find((item) => item.id === req.params.videoId);
  if (!project || !video) return void res.status(404).json({ error: 'Video generation not found' });
  const storyboard = project.storyboards.find((item) => item.id === video.storyboardId);
  if (!storyboard?.approved || storyboard.scenesRevision !== project.scenesRevision) return void res.status(409).json({ error: 'Video was not generated from the current approved storyboard' });
