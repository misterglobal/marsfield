...
const SEEDANCE_MODELS = modelIdsForFamily('seedance');
const NANO_BANANA_MODELS = modelIdsForFamily('nano-banana');
const IMAGE_MODELS = new Set(['general', 'nano-banana', 'recraft'].flatMap((family) =>
  Array.from(modelIdsForFamily(family as 'general' | 'nano-banana' | 'recraft'))
...
router.post('/generate/quote', authMiddleware, requireScope('generation:write'), rateLimit('quote'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (workflow === 'video-enhance' && VIDEO_ENHANCEMENT_MODELS.has(model)) {
      ...
      if (!object?.url || !object.mimeType?.startsWith('video/')) throw new Error('Owned video asset required');
      const metadata = await getRemoteVideoMetadata(object.url);
      ...
      if (!image) throw new Error('Owned image asset required');
      const inputMegapixels = await getRemoteImageMegapixels(image);
      ...
      billingParams.duration = duration;
      billingParams.formats = params.formats ? assertSocialResizeFormats(params.formats) : [assertSocialResizeFormat(params.format)];
...
router.post('/generate', authMiddleware, requireScope('generation:write'), rateLimit('generation'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!modelSupportsWorkflow(model, workflow)) {
    ...
    try {
      if (isSeedance) {
        ...
        const referenceImages = await resolveOwnedStorageObjects(
          user.id,
          params?.reference_image_ids || [],
          workflow === 'character-replace' ? 1 : 4,
          'image/',
        ...
        if (!VIDEO_ENHANCEMENT_MODELS.has(model)) throw new Error('Unsupported video enhancement model');
        ...
        const metadata = await getRemoteVideoMetadata(videoObject.url);
        billingParams.duration = metadata.duration;
        billingParams.input_megapixels = (metadata.width * metadata.height) / 1_000_000;
        billingParams.input_fps = metadata.fps;
        ...
        if (model === 'xai/grok-imagine-video-extension') {
          ...
          if (videoObject.mimeType !== 'video/mp4') throw new Error('Grok video extension requires an MP4 source');
          if (metadata.duration < 2 || metadata.duration > 15.05) throw new Error('Grok source video must be between 2 and 15 seconds');
          ...
          if (typeof prompt !== 'string' || !prompt.trim()) throw new Error('Grok extension requires a prompt');
          ...
          if (!['720p', '1080p', '4k'].includes(targetResolution)) throw new Error('Invalid Topaz target resolution');
          if (!Number.isInteger(targetFps) || targetFps < 15 || targetFps > 60) throw new Error('Topaz target FPS must be from 15 to 60');
          ...
          if (!Number.isFinite(scaleFactor) || scaleFactor < 1 || scaleFactor > 8) throw new Error('Crystal scale factor must be from 1 to 8');
        ...
        if (!IMAGE_UPSCALE_MODELS.has(model)) throw new Error('Unsupported image upscaling model');
        const image = await resolveOwnedStorageObject(user.id, req.body.image_storage_object_id, 'image/', 'Upscale image');
        if (!image) throw new Error('Image upscaling requires an owned image asset');
        const inputMegapixels = await getRemoteImageMegapixels(image);
        billingParams.input_megapixels = inputMegapixels;
        ...
        if (model === 'prunaai/p-image-upscale') {
          ...
          if (!Number.isInteger(target) || target < 1 || target > 128) throw new Error('Pruna target must be from 1 to 128 megapixels');
          if (!['jpg', 'png', 'webp'].includes(outputFormat)) throw new Error('Invalid Pruna output format');
          if (!Number.isInteger(outputQuality) || outputQuality < 0 || outputQuality > 100) throw new Error('Output quality must be from 0 to 100');
          ...
          if (!['x2', 'x4'].includes(upscaleFactor)) throw new Error('Google upscale factor must be x2 or x4');
          if (!Number.isInteger(compressionQuality) || compressionQuality < 1 || compressionQuality > 100) throw new Error('Compression quality must be from 1 to 100');
          ...
          if (![2, 4].includes(scaleFactor)) throw new Error('Clarity Pro scale factor must be 2 or 4');
          if (!Number.isFinite(creativity) || creativity < -10 || creativity > 10) throw new Error('Clarity creativity must be from -10 to 10');
          if (!['png', 'jpg'].includes(outputFormat)) throw new Error('Invalid Clarity output format');
        ...
        const formats = params?.formats ? assertSocialResizeFormats(params.formats) : [assertSocialResizeFormat(params?.format)];
        ...
        predictionInput = {
          ...
          formats,
          mode,
          aspect_ratio: formats.map((format) => socialResizeLabel(format)).join(', '),
        };
