...
router.get('/productions', authMiddleware, requireScope('projects:read'), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/productions/:id', authMiddleware, requireScope('projects:read'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) return void res.status(401).json({ error: 'Unauthorized' });
...
router.post('/productions', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const plan = planYoutubeProduction({ topic, audience, targetDurationMin, angleCount });
...
router.post('/productions/:id/create-project', authMiddleware, requireScope('projects:write'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) return void res.status(401).json({ error: 'Unauthorized' });
    const production = await prisma.youtubeProduction.findFirst({
      where: { id: req.params.id, userId: req.user.id },
    ...
    if (!production) return void res.status(404).json({ error: 'Production not found' });
    if (production.projectId) return void res.status(409).json({ error: 'Production already has a linked project' });

    const storyboard = Array.isArray(production.storyboard) ? production.storyboard as any[] : [];
    if (!storyboard.length) return void res.status(400).json({ error: 'Production has no storyboard scenes' });
